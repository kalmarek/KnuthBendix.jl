...
function Base.show(io::IO, ::MIME"text/plain", s::State)
    ...
    elseif isterminal(s)
        ...
        for (i, e) in enumerate(inedges(s))
            !isfailstate(e) &&
                println(io, "   ", " - with label $(i) from state $(name(e))")
        end
    else
        println(io, "State $(name(s))")
        println(io, " Edges entering:")
        for (i, fromst) in enumerate(inedges(s))
            !isfailstate(fromst) && println(
                io,
                "   ",
                " - with label $(i) from state $(name(fromst))",
            )
        end
        println(io, " Edges leaving:")
        for (i, tost) in enumerate(outedges(s))
            !isfailstate(tost) && println(
                io,
                "   ",
                " - with label $(i) from state $(name(tost))",
            )
        end
    end
end
...
function Automaton(abt::Alphabet, W::Type{<:AbstractWord} = Word{UInt16})
    ...
    failstate = S()
    return Automaton{length(abt),W}(
        [State(one(W), failstate)],
        abt,
        failstate,
        [0],
        S[],
...
function Base.push!(a::Automaton{N,W}, name::W) where {N,W}
    return (push!(states(a), State(name, failstate(a)));
    push!(stateslengths(a), length(name));
    a)
end
function Base.empty!(a::Automaton{N,W}) where {N,W}
    return (empty!(states(a)); empty!(stateslengths(a)); push!(a, one(W)); a)
end
...
"""
Base.@propagate_inbounds function replace(
    k::NTuple{N,T},
    val::T,
    idx::Integer,
) where {N,T}
...
"""
Base.@propagate_inbounds function updateoutedges!(
    src::State{N},
    dst::State{N},
    label::Integer,
) where {N}
...
Base.@propagate_inbounds function addedge!(
    ...
    @boundscheck 1 ≤ label ≤ N ||
                 throw(BoundsError(outedges(states(a)[src]), label))
    ...
    return addinedge!(dststate, srcstate)
end
...
"""
Base.@propagate_inbounds function removeedge!(
    a::Automaton{N},
    label::Integer,
    from::Integer,
    to::Integer,
    ...
    @boundscheck 1 ≤ label ≤ N ||
                 throw(BoundsError(outedges(states(a)[from]), label))
    ...
    return deleteat!(
        inedges(tostate),
        findfirst(isequal(fromstate), inedges(tostate)),
    )
end
...
Base.@propagate_inbounds function Base.deleteat!(a::Automaton, idx::Integer)
    ...
    for σ in outedges(state)
        isfailstate(σ) ||
            deleteat!(inedges(σ), findfirst(isequal(state), inedges(σ)))
    end
    ...
    while !isempty(indcs)
        for idx in indcs
            ...
            for (letter, state) in enumerate(outedges(σ))
                isfailstate(state) &&
                    addedge!(a, letter, σ, outedges(τ)[letter])
            end
        end
        i += 1
        resize!(indcs, 0)
...
"""
function makeindexautomaton(rws::RewritingSystem)
    return (a = Automaton(alphabet(rws)); makeindexautomaton!(a, rws))
end
...
function updateautomaton!(a::Automaton, rws::RewritingSystem)
    return makeindexautomaton!(a, rws)
end
